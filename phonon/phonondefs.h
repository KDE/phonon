...
template <typename T> static inline T *pGetPtrHelper(T *ptr) { return ptr; }
template <typename Wrapper> static inline typename Wrapper::pointer pGetPtrHelper(const Wrapper &p) { return p.data(); }
...
#define K_DECLARE_PRIVATE(Class) \
    inline Class##Private* k_func() { return reinterpret_cast<Class##Private *>(pGetPtrHelper(k_ptr)); } \
    inline const Class##Private* k_func() const { return reinterpret_cast<const Class##Private *>(pGetPtrHelper(k_ptr)); } \
