...
#warning two and one were inverted.....
            i = 1;
...
//    n->setRe
    ...
    v->set(0, 0, 0, 0);
    // bottom left
    (++v)->set(0, height(), 0, d->interface->frame()->height);
    // top right
    (++v)->set(width(), 0, d->interface->frame()->width, 0);
    // bottom right
    (++v)->set(width(), height(), d->interface->frame()->width, d->interface->frame()->height);
